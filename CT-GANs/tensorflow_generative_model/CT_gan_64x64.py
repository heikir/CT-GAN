...
DATA_DIR = '/home/bigdata/Downloads/imagenet'
if len(DATA_DIR) == 0:
    raise Exception('Please specify path to data directory in gan_64x64.py!')

LAMBDA_2 =2.0  # parameter LAMBDA2
Factor_M = 0.0  # factor M
MODE = 'wgan-ct' # dcgan, wgan, wgan-gp, lsgan
...
def Normalize(name, axes, inputs):
    if ('Discriminator' in name) and (MODE == 'wgan-ct'):
        if axes != [0,2,3]:
...
def GoodDiscriminator(inputs, dim,kp1,kp2,kp3):
    ...
    output2 = tf.reshape(output, [-1, 4*4*8*dim])
    output = lib.ops.linear.Linear('Discriminator.Output', 4*4*8*dim, 1, output2)
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    for device_index, (device, real_data_conv) in enumerate(zip(DEVICES, split_real_data_conv)):
        with tf.device(device):
            ...
            disc_real,disc_real_2 = Discriminator(real_data,DIM, 0.8,0.5,0.5)
            disc_real_,disc_real_2_ = Discriminator(real_data,DIM, 0.8,0.5,0.5)
            disc_fake,disc_fake_2 = Discriminator(fake_data,DIM, 0.8,0.5,0.5)
            ...
            if MODE == 'wgan':
                ...
                gradients = tf.gradients(Discriminator(interpolates,DIM,0.8,0.5,0.5)[0], [interpolates])[0]
